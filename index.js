...
Toolkit.run(async (tools) => {
    ...
    const commitMessage = process.env['INPUT_COMMIT-MESSAGE'] || 'bump ci: version bump to {{version}}';
    ...
    try {
        ...
        await tools.exec('git', [
            'config',
            'user.name',
            `"${process.env.GITHUB_USER || 'Autobump versions'}"`,
        ]);
        await tools.exec('git', [
            'config',
            'user.email',
            `"${process.env.GITHUB_EMAIL || 'gh-action-bump--autobump-version@users.noreply.github.com'}"`,
        ]);
